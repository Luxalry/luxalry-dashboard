...
`;
    ...
    async apiFetch(url, options = {}) {
        const isBackdoor = sessionStorage.getItem('backdoor_token') || sessionStorage.getItem('escalation_token');
        options.credentials = 'include';
        
        // Only set default content-type if not FormData
        if (!(options.body instanceof FormData)) {
            options.headers = Object.assign({ 'Content-Type': 'application/json' }, this.getAuthHeaders(), options.headers || {});
        } else {
        ...
        if (response.status === 401) {
            if (isBackdoor) {
                await this.logout();
                throw new Error("Escalation token expired or invalid.");
            }

            if (!this.refreshPromise) {
                this.refreshPromise = fetch(`${window.APP_CONFIG.API_BASE_URL}/api/admin?action=refresh`, {
                    method: 'POST',
                    credentials: 'include'
                }).then(async (res) => {
            ...
            try {
                ...
                options.headers['Authorization'] = 'Bearer ' + localStorage.getItem('admin_token');
